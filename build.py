...
def draw(df, code, name, asof, window=120, notes=None, info=None):
    ...
    if "macd_cross_mark" in d.columns:
        gold_idxs = [i for i, m in d["macd_cross_mark"].items() if m == 1]
        dead_idxs = [i for i, m in d["macd_cross_mark"].items() if m == -1]
        if gold_idxs:
            axm.scatter(gold_idxs, d["macd"].iloc[gold_idxs], marker="^", s=60,
                        color=CROSS_GOLD, edgecolors="white", linewidths=0.8,
                        zorder=10, label="골든크로스")
        if dead_idxs:
            axm.scatter(dead_idxs, d["macd"].iloc[dead_idxs], marker="v", s=60,
                        color=CROSS_DEAD, edgecolors="white", linewidths=0.8,
                        zorder=10, label="데드크로스")
